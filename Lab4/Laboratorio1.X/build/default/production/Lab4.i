...
bsf ((INTCON) and 07Fh), 3 ;PORTB change interrupt enable bit
bsf ((INTCON) and 07Fh), 2 ;timer0 overflow interrupt flag
...
reset0:
    ...
    movwf TMR0
    bcf ((INTCON) and 07Fh), 2 ;convertir a 0 el bit de overflow
    return

timer0:
    btfss ((INTCON) and 07Fh), 2 ;sumar cuando llegue al overflow de timer0
    goto $-1
...
actv:
    ;subrutona de incremento y decremento
    btfss PORTB, 0 ;revisa si se prende el boton 1
    incf PORTA ;incrementa
    btfss PORTB, 1 ;revisa si se prende el boton 2
...
config_clock:
    ...
    banksel OSCCON
    bcf ((OSCCON) and 07Fh), 6 ;se utiliza 010
    bsf ((OSCCON) and 07Fh), 5
    bcf ((OSCCON) and 07Fh), 4 ;250 Hz
    bsf ((OSCCON) and 07Fh), 0 ;activacion de oscilador interno
    return
